...
counties <- st_read("../tl_2016_us_county/tl_2016_us_county.shp",stringsAsFactors=FALSE)
...
for (icounty in 1:nrow(contig_counties)){
  i=which(abs(as.numeric(contig_counties$INTPTLON[icounty])-Lon) == min(abs(as.numeric(contig_counties$INTPTLON[icounty])-Lon)))
  j=which(abs(as.numeric(contig_counties$INTPTLAT[icounty])-Lat) == min(abs(as.numeric(contig_counties$INTPTLAT[icounty])-Lat)))
  contig_counties$SPI[icounty]=zplot[i,j,iMonth]
}
